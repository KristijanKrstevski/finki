...
public class CountryController {
    ...
    public ResponseEntity<?> addCountry(@RequestBody CreateCountryDTO countryDto) {
        return ResponseEntity.ok(countryApplicationService.create(countryDto));
    }

    @PutMapping("/edit/{id}")
    @Operation(summary = "Edit a country", description = "Edits an existing country's details")
    public ResponseEntity<?> editCountry(@RequestBody CreateCountryDTO countryDto, @PathVariable Long id) throws Exception {
